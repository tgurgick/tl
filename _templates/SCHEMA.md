...
When required and no independent verifier is available, the builder **stops at `tests/`** (`status: blocked`) instead of self-verifying: it writes `outcome/FEEDBACK.md`, snapshots `git diff > outcome/BUILDER.diff`, sets spec frontmatter `awaiting_verifier: true` + `requested_at: YYYY-MM-DD`, and writes a minimal `VERIFY.md` request in the spec folder (builder, date, anything to flag). **Canonical verifier eligibility** is a valid `outcome/HANDOFF.json` in `tests/` — the manifest is completion proof; `awaiting_verifier` / `VERIFY.md` / cockpit `_metrics/verify-requests/*.json` are migration stamps and **routing hints**, not a second queue of truth. Prefer **one** scheduled verify drain (`automation.verify: true` → `tl-worker --mode verify`); `tl verify [ws] [--agent <name>]` and `tl verify --execute` remain supported recovery launchers with the **same** read-only lease + ALIGNMENT contract. The scheduled tick claims at most one eligible spec through a configured `verifier_lanes` entry, never the builder's own, under a per-spec lock at `_metrics/verify-locks/<slug>.lock`. Cockpit **Dispatch verify** only writes request files — the UI/server never spawn agent CLIs. A clean isolated pass advances `tests → in-review` with `verified_by` / `verification_type: independent`. Mutation proposals become `human-decision-required` and stay at `tests/` until an explicit human choice (`authorize-fix-forward` or `kick-back`) — never auto-applied; fix-forward requires human authorization and a **separate agent**. Failures leave the spec in `tests/` with an auditable `blocked_reason`. Unsafe Gemini configs (missing `isolated`/`sandbox`, `allow_network: true`, `--dangerously-skip-permissions`) are rejected loudly at config/read time. A section absent from `TRIAGE.yml` means the gate is not enforced (pre-gate workspaces unchanged). Agents never move work to `done/` — that hop is human-only.
...
**Migration / legacy.** Specs that already carry `outcome/FEEDBACK.md` + `outcome/BUILDER.diff` (and often `awaiting_verifier` / `VERIFY.md`) **without** `HANDOFF.json` are **legacy**: `classifyHandoff` returns `{ kind: 'legacy', legacy: true }` and remains readable for grandfathered verify/review paths. New handoffs must write V1. **Prepared-handoff recovery** (`tl recover` / `recoverPreparedHandoff`) requires a **valid committed manifest** and an **expired builder lease** (or the documented no-lease grace path) — artifacts alone are insufficient; `FEEDBACK`+diff without a manifest is `no-manifest`, not recoverable completion. Recovery reuses the terminal manifest byte-identically (`reuse_only`) and refuses before stamp/overwrite/move if digests no longer match (`manifest-invalidated`). Opt-in enforcement (e.g. `verification.require_handoff_binding` on `canAdvanceToReview`) belongs to a follow-up wire-up — absent policy, legacy work stays valid. Do not retro-stamp historical `tests/` / `in-review/` / `done/` folders.
...
experiments:            # optional — experiment routing policy (lib/experiment-policy.js)
  ...
  auto_initiate_rate: 1.0   # 0..1 — fraction of eligible fresh claims that initiate (Bernoulli per claim); absent/garbage = 1.0 (every eligible claim), 0 = never
  ...
  auto_initiate_allow_test_command: false  # opt-in: let a drain-wide --test-command run LIVE on auto-initiated judges (default: refused, tests_pass stays null)
automation:             # optional — tl up's schedule profile (lib/automation.js); open is an alias
...
**The auto-initiation dial (`auto_initiate`).** The one exception to "the policy never queues" — and it is a *separate opt-in on top of* `enabled`: shadow experiments spin up without a human typing `tl experiment queue` only when **both** `enabled: true` and `auto_initiate: true` are literal. Absent fields (or the whole section) = fully inert — zero writes, zero behavior change. When on, the headless worker tick (`lib/worker.js` `maybeAutoInitiateExperiment`, called immediately after the tick commits a fresh canonical claim — lock + prompt on disk — and before the agent spawn) consults `decideRouting` and queues the returned cohort through the ordinary `queueExperiment` path; the `tl run` claim path is a follow-up spec. The hook is **best-effort and failure-silent toward canonical work**: a broken experiment path is caught, logged, and skipped — it never stops or delays a claim (and dry-run ticks and continuation resumes never initiate; only fresh ready picks do). `auto_initiate_lanes` allowlists the candidate pool the policy routes over (matched against candidate ids and `agent_tool`s; empty = all configured candidates). The two budget caps — `auto_initiate_max_concurrent` (auto experiments with any non-terminal candidate row) and `auto_initiate_daily_max` (auto experiments created per UTC day) — **hold** new auto-experiments with a visible reason when exhausted; they never cancel running ones. Every decision (initiated / held / skipped / error) lands in `auto-initiation-log.jsonl` with the policy inputs that drove it (below); auto-created experiments are downstream-indistinguishable from manual ones — same artifacts, same judge path — except `initiated_by: "policy"` in `EXPERIMENT.md` frontmatter, written atomically at creation (absent = `"human"`, the manual CLI/UI paths). Auto-*application* stays banned: this dial only ever creates experiments.

**The sample rate (`auto_initiate_rate`).** How *much* of the claim stream becomes shadow measurement: a number in `[0, 1]` — each eligible fresh claim draws one Bernoulli trial at this rate **before** routing and before the budget gate. A miss is logged `skipped` with reason `sample_rate` (level `debug`, `sample_draw` in the policy inputs) and costs nothing: no priors read, no budget hold, and it never counts against the daily/concurrent budgets (only `initiated` rows do). Fallback-on-garbage to `1.0` — absent or junk means "every eligible claim", exactly the pre-rate behavior, and `1.0` draws nothing at all; `0` never initiates. This is a different knob from `explore_rate`, which decides *which* primary the routing policy picks once an initiation is happening — neither substitutes for the other. Unattended shadows need **both halves**: `auto_initiate` (+ rate) only *queues* on claim; the queued rows sit until something drains them, so pair it with `automation.experiment: drain` (below) — queue alone is not enough.

**Auto-shadow safety contract.** Auto-initiated experiments are shadow attempts, end to end: they write **only** under `_experiments/` (isolated worktree/clone workdirs, candidate/evaluation artifacts) and append-only `_metrics/` logs; they never edit the workspace's canonical stage folders (`triage/`…`done/`), never move or claim specs, and **winner application remains an explicit human action** — no worker, drain, or automation tick imports the apply path (tests enforce it). The residual risk on an unattended path is host execution, and it fails closed keyed off the `initiated_by: "policy"` stamp (`lib/env-policy.js`): **`unsafe_host_exec` is refused on auto-initiated rows** regardless of row flags or a drain-wide `--unsafe-host-exec` (the attempt terminates `failed` with a concrete reason before any command runs), and **auto-initiated judges refuse a live `--test-command`** by default — scores come from artifacts and the rubric; `tests_pass` stays `null` (declared unavailable, never a silent green) unless the workspace opts in with the literal `auto_initiate_allow_test_command: true`. Manual (`initiated_by` absent) experiments keep the shipped opt-in contract unchanged.

**The automation profile (`automation`).** An optional section that makes `tl up <workspace>` the one-command operating path (`tl open` is an alias only — same command; see `docs/canonical-e2e-path.md`): it declares the workspace's headless schedule instead of N hand-written crons (`lib/automation.js`; generator details in `docs/headless-lanes.md`). Absent section — or `enabled` anything but literal `true` — means **no behavior change**: `tl up` still starts the UI and prints the next human action, but installs nothing (calm default). When enabled, `tl up` installs or refreshes a **single per-workspace schedule** (one launchd plist on macOS, one printed cron line elsewhere — `tl up --print-schedule` emits both, paste-ready) that every `interval_minutes` ticks each lane in `lanes` sequentially via `bin/tl-worker.js <ws> --agent <lane>`. Every listed lane **must** already have `lanes.<name>.command` — a missing command is a hard error with a fix hint, never a silently-green schedule. `verify: true` appends **one primary** isolated verify tick (`bin/tl-worker.js <ws> --mode verify`) that claims at most one awaiting-verifier spec through `verification.verifier_lanes` (builder exclusion + per-spec lock); it requires at least one safe verifier lane or the profile is misconfigured. Interactive `tl run` / `tl verify` remain supported recovery launchers with identical contracts; verify-request files are routing hints, not queue truth.
...
The workspace `PAUSE` file remains the kill switch: the schedule keeps firing but every lane tick exits `2` without spawning, and `tl up` reports the workspace as paused. `tl up` never claims or moves specs — it schedules ticks; the ticks' spawned sessions (and the isolated verifier runner) do the work, and everything still pools at the human review gate. Agents never move work to `done/`.
...
`reflect-review-log.jsonl` (written by cockpit `/api/reflect-decision` and `tl reflect-decision`) records one line per **explicit human** disposition of a `/tl reflect` proposal. Viewing the proposal markdown alone must never write this file. Fields: `date` (ISO timestamp of the decision), `proposal_id` (stable id `reflect-YYYY-MM-DD` derived from the proposal artifact/log date), `proposal_date` (`YYYY-MM-DD`), `path` (`_metrics/reflect-YYYY-MM-DD.md`), `action` (`reviewed` `dismissed` `applied`), `actor` (who decided — `human-cockpit` / `human-cli` / a named `--by`), `via` (`cockpit` `cli`), optional `note`. Append-only: last line per `proposal_id` wins for desk unread derivation (`lib/reflect-desk.js`); history is never rewritten. Pre-marker proposals (no row yet) still degrade via the 14-day freshness window so the desk cannot accumulate forever. `applied` records that the human already applied the TRIAGE.yml change — the writer never auto-applies.
...
`auto-initiation-log.jsonl` records one line per auto-initiation decision (`experiments.auto_initiate` dial — written only when the dial is on; an off dial writes nothing). Fields include: `date` (ISO timestamp), `spec` (workspace-relative path of the claimed spec), `decision` (`initiated` `held` `skipped` `error`), `level` (`info` for initiations, budget holds, and errors; `debug` for policy "no" decisions and sample-rate misses), `initiated_by` (always `"policy"` — this log only exists for the policy path), `experiment_id` (the created experiment, else `null`), `reason` (human-readable; budget holds carry the visible exhaustion reason; sample-rate misses carry the literal `sample_rate`), `policy` (the inputs that drove the decision — the training signal for priors: dial settings including `auto_initiate_rate`, candidate ids, `context_key`, `primary` `{id, source, reason}`, `shadows`, `shadow_mode`, `scores`, `explore_rate`, `min_samples_to_route`; sample-rate misses record `sample_draw` — the Bernoulli draw, `null` at rate `0` where nothing is drawn), and `budget` (`daily_used`/`daily_max`, `concurrent_used`/`max_concurrent`). Initiations also appear in `experiment-log.jsonl` as a normal `queued` transition with reason `experiment queued (policy)`.
