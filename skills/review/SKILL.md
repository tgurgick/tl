...
- **On accept only — append one `benchmark-log.jsonl` record** via `lib/benchmark-log.js` (`buildBenchmarkRecord` + `appendBenchmarkRecord`) from the post-move `done/<slug>/SPEC.md`, `outcome/FEEDBACK.md`, workspace name, and `intentGoalIds` of the spec's intent file. Copy `spec_hash` from frontmatter — never recompute. The append is idempotent and must be failure-silent (never block accept). Kick-back does not write a benchmark row.
