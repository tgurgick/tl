...
- A completed spec must have `outcome/FEEDBACK.md` and `outcome/ALIGNMENT.md` (the cross-model verdict) before it moves to `in-review/`.
- The cross-model check is advisory-with-remediation and bounded (~2 rounds); it never removes the human gate. Unresolved concerns advance as `residual-concerns` for the human, they don't block.
