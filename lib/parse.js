// lib/parse.js — the one YAML-subset parser + frontmatter splitter.
//
// Canonical for the whole tool: bin/tl.js and ui/server.js both require this so
// the parsing rules can't drift into three hand-rolled copies. It handles the
// shapes tl actually uses — nested maps, block lists, inline arrays, scalars,
// and `#` comments — not general YAML. Node stdlib only; zero dependencies.
    ...
    while (idx < lines.length && lines[idx].indent === indent && !lines[idx].content.startsWith('- ')) {
      ...
      if (m[2] !== undefined && m[2] !== '') { obj[m[1]] = parseScalar(m[2]); idx++; }
        ...
        if (idx < lines.length && lines[idx].indent > indent) {
          ...
          // YAML permits an "indentless" sequence under a mapping key:
          //   depends_on:
          //   - specs/foo/
          // PyYAML emits this shape. Consume the same-indent list as the
          // preceding empty key, then continue parsing later map fields.
          const [v, ni] = parseList(idx, indent);
          obj[m[1]] = v; idx = ni;
        } else obj[m[1]] = null;
      }
    }
