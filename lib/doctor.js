...
function classifyLifecycle(wsDir, { now = Date.now(), specs = null, cfg = null } = {}) {
  ...
  for (const s of allSpecs) {
    ...
    const slug = slugFromPath(s.path);
    if (!slug) continue;
    ...
    if (awaiting.length) {
      ...
      if (blocked.length === awaiting.length) {
        return {
          ...
          builder_exclusion: { blocked_slugs: blocked.map(s => slugFromPath(s.path)).filter(Boolean) },
        };
      }
    }

    // busy: verify lock held by this lane
