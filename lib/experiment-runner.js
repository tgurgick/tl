...
//   config.env          extra environment (merged over process.env)
...
function runProviderCandidate(providerKey, workspaceDir, row, opts = {}) {
  ...
  try {
    ...
    return {
      ...
      agentModel: resultModelFromConfig(row, providerKey),
      agentModelSource: row.agent_model_requested ? 'requested' : (providerKey === 'cursor' ? 'unknown' : 'reported'),
      agentModelAuto: providerKey === 'cursor' && !row.agent_model_requested,
    };
  } finally {
    removeIsolatedWorkdir(repo, workdir, iso.mode);
    events.push({ type: 'status', summary: `Removed isolated ${iso.mode}.` });
...
function runCandidate(workspaceDir, row, opts = {}) {
  ...
  const visibility = resolveModelVisibility({
    agent_tool: row.agent_tool,
    agent_model_requested: row.agent_model_requested,
