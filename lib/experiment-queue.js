...
function normalizeCandidate(c, i) {
  ...
  return {
    ...
    // `complete`; `estimated_cost_usd` powers the budget stop. Provider
    // adapters (codex/gemini/claude/cursor — lib/experiment-runner.js
    // PROVIDERS) take STRUCTURED override fields — prompt, profile, sandbox,
    // extra_flags — never string concatenation into a shell.
    config: {
      ...(src.command ? { command: String(src.command) } : {}),
      ...(src.env && typeof src.env === 'object' ? { env: src.env } : {}),
      // Trust-boundary fields (lib/env-policy.js): `unsafe_host_exec: true`
      // is the explicit, auditable opt-in the shell runner requires (a
      // worktree isolates the checkout, not the machine); `pass_env` names
      // ambient variables to pass through the credential scrub.
      ...(src.unsafe_host_exec === true ? { unsafe_host_exec: true } : {}),
      ...(Array.isArray(src.pass_env) ? { pass_env: src.pass_env.map(String) } : {}),
      ...
      ...(src.prompt ? { prompt: String(src.prompt) } : {}),
      ...(src.profile ? { profile: String(src.profile) } : {}),
      ...(src.sandbox ? { sandbox: String(src.sandbox) } : {}),
      ...(Array.isArray(src.extra_flags) ? { extra_flags: src.extra_flags.map(String) } : {}),
    },
  };
}
...
//   initiatedBy     provenance stamp written into EXPERIMENT.md frontmatter
//                   ('policy' = the unattended auto-initiation dial; omitted
//                   = human — the manual CLI/UI paths write nothing). Written
//                   ATOMICALLY with creation so the auto-path safety refusals
//                   (lib/env-policy.js keying off initiated_by) can never
//                   miss a stamp.
...
function queueExperiment(workspaceDir, opts = {}) {
  ...
  fs.writeFileSync(path.join(expDir, 'EXPERIMENT.md'), [
    ...
    'status: "queued"',
    `created: "${created}"`,
    ...(opts.initiatedBy ? [`initiated_by: "${String(opts.initiatedBy)}"`] : []),
...
// lane. Judge rows are never surfaced here — they drain through
// `judgeLaneRows` below, after every candidate run in the experiment is over.
...
// The queued JUDGE rows a worker for `agentTool` may claim — same lane
// discipline as candidates: a judge row lands in the lane named by the
// experiment's `judge_tool`, and only that lane's worker executes it.
function judgeLaneRows(rows, agentTool) {
  const lane = String(agentTool || '').toLowerCase();
  return rows.filter(r => r.status === 'queued' && r.role === 'judge'
    && String(r.agent_tool || '').toLowerCase() === lane);
}
...
// Release a claim by removing its attempt marker — used when a run fails
// mid-flight so a later explicit retry can claim the same attempt number
// again instead of the marker pinning it forever. Best effort, never throws.
function releaseClaim(workspaceDir, row) {
  const marker = path.join(queueDir(workspaceDir), 'claims',
    `${row.experiment_id}--${row.candidate_id}--${row.attempt}.claim`);
...
// into real experiments + rows. Two runtimes bridge:
//   fixture  the deterministic proof cohort (fixture lanes, fixture judge);
//   local    a real local adapter run — the request must carry the two
//            minimal fields the cockpit form collects: `runner` (a registered
//            RUNNERS lane: codex | gemini | claude | cursor | shell) and
//            `repo` (the repo candidates run against, isolated worktree/clone).
//            All candidates land in that runner's lane; per-candidate models
//            ride in `models`; `command` is required for (and only used by)
//            the shell runner.
// A processed request is rewritten with status "accepted" + the experiment id
// (or "invalid" + the error) — malformed requests are marked invalid, NEVER
// silently dropped. Unknown future runtimes stay `left-queued` (reported every
// pass) for a later slice; unparseable JSON is reported invalid in the results
// without rewriting the file (it may be a corrupted audit record — the file is
// the audit trail, never deleted, never clobbered blind).
function processQueueRequests(workspaceDir, opts = {}) {
  ...
  for (const f of fs.readdirSync(dir).sort()) {
    ...
    try { cfg = JSON.parse(fs.readFileSync(file, 'utf8')); } catch {
      results.push({ file: f, status: 'invalid', reason: 'unparseable JSON — file left untouched' });
      continue;
    }
    if (!cfg || cfg.status !== 'queued') continue;
    const runtime = String(cfg.runtime || '');
    if (runtime !== 'fixture' && runtime !== 'local') {
    ...
    const markInvalid = (reason) => {
      fs.writeFileSync(file, JSON.stringify({ ...cfg, status: 'invalid', error: String(reason).slice(0, 300) }, null, 2) + '\n');
      results.push({ file: f, status: 'invalid', reason: String(reason) });
    };
    ...
    if (runtime === 'fixture') {
      ...
      // runtime "local" — validate the bridge fields loudly; a malformed
      // request becomes `invalid` with the exact reason, never a silent skip.
      const runner = String(cfg.runner || '').trim().toLowerCase();
      const repo = String(cfg.repo || '').trim();
      // Lazy require: keeps this module loadable without the runner (same
      // seam discipline as drainQueue's runCandidate injection).
      const { hasRunner } = opts.hasRunner ? { hasRunner: opts.hasRunner } : require('./experiment-runner');
      if (!runner) { markInvalid('local request requires "runner" (codex | gemini | claude | cursor | shell)'); continue; }
      if (runner === 'fixture' || !hasRunner(runner)) { markInvalid(`unknown runner "${runner}" — no registered local adapter lane`); continue; }
      if (!repo) { markInvalid('local request requires "repo" — the repo candidates run against (isolated worktree/clone)'); continue; }
      if (!fs.existsSync(repo)) { markInvalid(`repo not found: ${repo}`); continue; }
      if (runner === 'shell' && !String(cfg.command || '').trim()) { markInvalid('runner "shell" requires "command"'); continue; }
      const base = {
        agent_tool: runner,
        repo,
        ...(String(cfg.command || '').trim() ? { command: String(cfg.command) } : {}),
        ...(cfg.prompt ? { prompt: String(cfg.prompt) } : {}),
        ...(cfg.profile ? { profile: String(cfg.profile) } : {}),
        // Explicit trust opt-in rides the request only when literally true —
        // a shell row without it queues fine but fails closed at run time
        // (lib/experiment-runner.js), with the full artifact set as evidence.
        ...(cfg.unsafe_host_exec === true ? { unsafe_host_exec: true } : {}),
      };
      candidates = [
        { id: cfg.primary || `${runner}-a`, role: 'primary', agent_model: models[cfg.primary] || models.primary || null, ...base },
        ...(Array.isArray(cfg.shadows) ? cfg.shadows : []).map(s =>
          ({ id: s, role: 'shadow', agent_model: models[s] || null, ...base })),
      ];
    }
    try {
      const created = queueExperiment(workspaceDir, {
        spec: cfg.tl_spec || '',
        repoDir: runtime === 'local' ? String(cfg.repo) : undefined, // base_commit from the run repo
        // The request filename is unique in the queue dir (the UI 409s name
        // collisions), so it yields a collision-proof experiment id even when
        // two same-spec requests bridge in the same second-resolution pass.
        experimentId: ('exp-' + f.replace(/\.json$/, '')).replace(/[^A-Za-z0-9._-]+/g, '-').slice(0, 80),
        candidates,
        judge: { id: cfg.judge || 'fixture-judge', agent_tool: 'fixture' },
        // null must become undefined: +null is 0, and a 0-minute timeout on a
        // row clamps a real run to a ~1ms spawn timeout (instant timed_out).
        budgetUsd: cfg.budget_usd == null ? undefined : cfg.budget_usd,
        timeoutMinutes: cfg.timeout_minutes == null ? undefined : cfg.timeout_minutes,
...
//   4. OPT-IN (`judges: true`): claim + execute queued judge rows in this
//      agent's lane headlessly (deterministic checks in code; model-judgment
//      dimensions land in a lane-agnostic JUDGE-BRIEF.md — see
//      lib/experiment-judge.js). Opt-in at this layer keeps the existing
//      candidate-only drain contract unchanged for library callers; the CLI
//      (`tl experiment drain`) turns it on by default, with `--skip-judges`
//      leaving judge rows queued for the interactive skill path.
...
function drainQueue(workspaceDir, opts = {}) {
  ...
  // Judge execution — same claim discipline as candidates: the exclusive
  // marker decides races, and a judge row queued by THIS pass (step 3) is
  // executed in the same pass when it lands in this worker's lane. A judge
  // that fails mid-run is marked with a fault status and its claim released;
  // it never half-writes an evaluation (staged-rename inside runJudge). The
  // experiment stays awaiting_evaluation for an explicit retry or the skill.
  const judged = [];
  if (opts.judges === true) {
    // Injected for tests; required lazily so this module stays loadable
    // without the judge (mirrors the runCandidate seam above).
    const runJudge = opts.runJudge || require('./experiment-judge').runJudge;
    for (const row of judgeLaneRows(readQueueRows(workspaceDir), agent)) {
      const claimed = claimRow(workspaceDir, row, agent, opts);
      if (!claimed) { judged.push({ row, status: 'skipped', winner: null, reason: 'claim lost — another worker holds it' }); continue; }
      let outcome;
      try {
        outcome = runJudge(workspaceDir, claimed, opts);
      } catch (e) {
        // runJudge contains its own failures; this catch is the belt for a
        // judge *crash* — the row still reaches a terminal fault status.
        outcome = { status: 'failed', winner: null, reason: `judge crashed: ${String(e.message).slice(0, 300)}` };
      }
      const status = outcome && outcome.status === 'succeeded' ? 'succeeded' : 'failed';
      if (status !== 'succeeded') releaseClaim(workspaceDir, claimed);
      markRow(workspaceDir, claimed, status, { reason: (outcome && outcome.reason) || null, now: opts.now });
      judged.push({ row: claimed, status, winner: (outcome && outcome.winner) || null, reason: (outcome && outcome.reason) || null });
    }
  }

  return { agent, requests, ran, judges, judged };
}
