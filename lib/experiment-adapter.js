...
// ---------------------------------------------------------------------------
//
// The open core only knows how to read/write local routing priors as an
// append-only JSONL log. A future private/hosted learned model is just another
// policy adapter satisfying this same tiny interface, so it can be swapped in
// without the core depending on it.

const ROUTING_PRIORS_FILE = 'routing-priors.jsonl';

// The local, file-backed policy: no learned model, just append-and-tally over
// `routing-priors.jsonl`. `choose` is a trivial baseline (first adapter, or the
// one with the best observed win-rate if priors are supplied). A private model
// would implement the same { name, choose, record } shape.
function createLocalRoutingPolicy(options = {}) {
  return {
    name: 'local-priors',
    priorsFile: options.priorsFile || ROUTING_PRIORS_FILE,

    // Format one prior row for the append-only log. The core writes these; it
    // does not learn from them beyond simple tallies.
    formatPriorRow(row = {}) {
      return {
        ts: row.ts || new Date().toISOString(),
        task_type: str(row.task_type, ''),
        agent_tool: str(row.agent_tool, ''),
        agent_model: str(row.agent_model, ''),
        outcome: str(row.outcome, ''), // 'won' | 'lost' | 'failed'
        utility: typeof row.utility === 'number' ? row.utility : null,
      };
    },

    // Pick an adapter for a task. Baseline: prefer the highest win-rate seen in
    // `priors`, else fall back to the first candidate. Deterministic and
    // model-free by design.
    choose(candidates, priors = []) {
      if (!Array.isArray(candidates) || candidates.length === 0) return null;
      if (!Array.isArray(priors) || priors.length === 0) return candidates[0];
      const tally = {};
      for (const p of priors) {
        const key = p.agent_tool;
        if (!key) continue;
        tally[key] = tally[key] || { wins: 0, total: 0 };
        tally[key].total += 1;
        if (p.outcome === 'won') tally[key].wins += 1;
      }
      ...
      for (const c of candidates) {
        ...
        const t = tally[name];
        const rate = t && t.total ? t.wins / t.total : 0;
