...
const { specSlug, activeConflicts, selectBatch } = require('./batch');
...
// lane another agent's resume brief.
function pickWork({ specs, continuations, lane, dirtyPaths = [] }) {
  const eligible = continuations.filter(c => continuationEligible(c.spec, lane));
  if (eligible.length) return { kind: 'continuation', picked: eligible[0].file, spec: eligible[0].spec };
  if (continuations.length) return { kind: 'none', picked: null, reason: 'no_continuation' };

  // Same lane + conflict discipline as `tl run --agent <lane>`.
  ...
  const { batch } = selectBatch(ready, doneSlugs, { active });
  if (!batch.length) return { kind: 'none', picked: null, reason: 'no_ready' };
  return { kind: 'ready', picked: batch[0].path, spec: batch[0] };
}
...
function tick(opts) {
  ...
  const pick = pickWork({ specs, continuations, lane, dirtyPaths });
  if (!pick.picked) {
