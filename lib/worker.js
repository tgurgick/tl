...
function finalizeBuilderHandoff(opts = {}) {
  ...
  // reuse_only (recovery): validate-and-reuse the existing terminal manifest, or
  // refuse BEFORE any stamp, overwrite, or move. Builder retries leave this off
  // so a drifted manifest can still be re-prepared under the builder's lease.
  const reuseOnly = opts.reuseOnly === true || opts.reuse_only === true;
  const manifestAbs = path.join(specDir, 'outcome', 'HANDOFF.json');
  if (reuseOnly) {
    if (!fs.existsSync(manifestAbs)) {
      return refuseTraced(leaseRefusal('manifest-invalidated', {
        slug,
        detail: 'reuse_only finalize requires an existing terminal HANDOFF.json — refused before stamp, overwrite, or move',
      }));
    }
    const pre = validateHandoff({
    ...
    if (!pre.ok) {
      return refuseTraced({
        ...pre,
        reason: 'manifest-invalidated',
        slug,
        cause: pre.reason || null,
        manifest_committed: true,
        detail: 'reuse_only: existing terminal manifest is invalid or artifact bytes drifted — refused before stamp, overwrite, or move'
          + (pre.reason ? ' (cause: ' + pre.reason + ')' : ''),
      });
    }
  }
  ...
  // reuse_only never takes the recreate branch — refuse instead.
  const manifestOpts = {
  ...
  if (fs.existsSync(manifestAbs)) {
    ...
    if (v.ok) { manifest = v.manifest; reusedManifest = true; }
      ...
      return refuseTraced(leaseRefusal('manifest-invalidated', {
        ...
        cause: v.reason || null,
        manifest_committed: true,
        detail: 'reuse_only: terminal manifest invalidated after stamps — refused without overwrite or move'
          + (v.reason ? ' (cause: ' + v.reason + ')' : ''),
      }));
    } else {
    ...
    return refuseTraced(leaseRefusal('manifest-invalidated', {
      slug,
      detail: 'reuse_only: terminal HANDOFF.json missing after stamps — refused without create or move',
    }));
  } else {
...
//   - SAMPLED: `experiments.auto_initiate_rate` (0..1, default 1.0) draws a
//     Bernoulli trial per fresh claim before routing — unattended shadows can
//     cover N% of claims without a human clicking between steps. Misses are
//     logged `skipped` / `sample_rate` and never touch budgets.
//   - SHADOW-SAFE: auto experiments write only under `_experiments/` and
//     `_metrics/`; they never move canonical stage folders and never apply
//     winners. The host-execution residual risk is closed downstream, keyed
//     off the `initiated_by: "policy"` stamp (lib/env-policy.js):
//     unsafe_host_exec is refused on auto rows regardless of row flags, and
//     live judge test commands default off unless
//     `experiments.auto_initiate_allow_test_command: true`.
...
//
// `auto_initiate_rate` is the SAMPLE rate — the fraction of eligible fresh
// claims that initiate a shadow experiment (a Bernoulli trial per claim,
// drawn before routing; see the sample-rate gate below). It is a number in
// [0, 1]; absent or garbage falls back to 1.0 — "every eligible claim", the
// exact pre-rate behavior — and the dial being off reads as rate 0 (nothing
// initiates either way). Distinct from `explore_rate`, which is about WHICH
// primary the routing policy picks, never about whether to initiate.
function autoInitiateDial(expConfig) {
  ...
  const enabled = src.auto_initiate === true;
  const r = src.auto_initiate_rate;
  const rate = (typeof r !== 'boolean' && r !== null && r !== ''
    && Number.isFinite(+r) && +r >= 0 && +r <= 1) ? +r : 1.0;
  return {
    enabled,
    rate: enabled ? rate : 0,
...
//
//   opts: { wsDir, specPath, spec, triageCfg, repoDir, now, rng, sampleRng, print }
...
//     rng        injectable RNG shared with decideRouting's explore roll
//     sampleRng  injectable RNG for the sample-rate gate only (falls back to
//                rng, then Math.random) — lets tests pin the Bernoulli draw
//                without disturbing the routing roll
//
...
function maybeAutoInitiateExperiment(opts) {
  ...
  try {
    ...
    // Sample-rate gate — one Bernoulli trial per fresh claim, drawn BEFORE
    // budgets and BEFORE decideRouting, so a miss costs nothing: no priors
    // read, no budget hold, and (because only `initiated` log rows are ever
    // counted) no budget consumption. Rate 1 draws NOTHING — the pre-rate
    // code path byte-for-byte, including the rng sequence decideRouting sees;
    // rate 0 skips deterministically without a draw. The RNG is injectable
    // for tests (opts.sampleRng, falling back to opts.rng, then Math.random).
    if (dial.rate < 1) {
      const draw = dial.rate > 0 ? Number((opts.sampleRng || opts.rng || Math.random)()) : null;
      if (draw === null || !(draw < dial.rate)) {
        appendAutoInitiationLog(wsDir, {
          date: nowIso, spec: specPath, decision: 'skipped', level: 'debug',
          initiated_by: 'policy', experiment_id: null, reason: 'sample_rate',
          policy: { ...dialInputs, sample_draw: draw },
        });
        return { decision: 'skipped', experiment_id: null, reason: 'sample_rate' };
      }
    }
    ...
    // `initiated_by: "policy"` (absent = "human" — the manual `tl experiment
    // queue`/UI paths) is written ATOMICALLY with EXPERIMENT.md so the
    // auto-path safety refusals (lib/env-policy.js: unsafe_host_exec refused,
    // live test commands default-off) can never miss the stamp.
