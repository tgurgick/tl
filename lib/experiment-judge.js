...
//     AUTO-INITIATED experiments (initiated_by: policy) additionally refuse a
//     drain-wide --test-command unless the workspace opted in with
//     `experiments.auto_initiate_allow_test_command: true` — an unattended
//     judge must not execute a nobody-reviewed patch on this host.
...
const { scrubEnvironment, autoTestCommandAllowed, isAutoInitiated } = require('./env-policy');
...
function checkPatchInIsolation(workspaceDir, row, meta, patch, opts = {}) {
  ...
  try {
    ...
    try {
      ...
      let testCommand = opts.testCommand ? String(opts.testCommand) : null;
      // Auto-initiated experiments (initiated_by: policy) default to NO live
      // test command — the drain runs unattended, so a drain-wide
      // --test-command must not execute a nobody-reviewed candidate patch on
      // this host. tests_pass stays null (declared unavailable — never a
      // silent green), and only the workspace's explicit
      // `experiments.auto_initiate_allow_test_command: true` widens it. The
      // decision lives in lib/env-policy.js (the shared trust boundary); an
      // unreadable config fails closed.
      if (testCommand && isAutoInitiated(meta)) {
        let expCfg = {};
        try { expCfg = require('./experiment-policy').readExperimentsConfig(workspaceDir); } catch { expCfg = {}; }
        if (!autoTestCommandAllowed(meta, expCfg)) {
          return {
            patch_applies: true,
            tests_pass: null,
            note: 'auto-initiated experiment (initiated_by: policy): live test command refused — unattended judges score from artifacts only; opt in with experiments.auto_initiate_allow_test_command: true',
          };
        }
      }
