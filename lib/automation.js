...
const { readQueueRows } = require('./experiment-queue');
const { diagnoseWorkspace } = require('./doctor');
...
function automationStatus({
  wsDir, wsName, root, cfg, nodeBin = process.execPath, home = process.env.HOME || '',
  which = null, env = process.env, now = Date.now(),
}) {
  ...
  // Shared health classifier (lib/doctor.js) — stuck-at-tests + capacity/lifecycle.
  // which=null skips PATH probes inside automationStatus (install path stays
  // offline); callers that want reachability pass an injectable which.
  let health = null;
  try {
    health = diagnoseWorkspace(wsDir, { cfg, specs, which, env, now });
  } catch { health = null; }

  const stuckAtTests = health
    ? health.stuck_at_tests
    : specs.filter(s =>
      s.stage === 'tests' &&
  ...
  return {
    state, signal, automation, issues, paused, lanes, stuckAtTests, installed,
    health,
  };
}

// Cheap, offline lane probe for `tl up`: configured command → PATH lookup,
// workspace claims/locks → busy, otherwise a concrete queue pointer. `which`
// is injected so tests never depend on host-installed agent CLIs.
function laneAvailability({ wsDir, cfg, which = () => '', env = process.env }) {
  const configured = cfg && cfg.lanes && typeof cfg.lanes === 'object' && !Array.isArray(cfg.lanes)
    ? Object.keys(cfg.lanes).filter(validLaneName).sort() : [];
  let specs = [];
  try { specs = readWorkspaceSpecs(wsDir); } catch { /* unreadable workspace */ }
  let queueRows = [];
  try { queueRows = readQueueRows(wsDir); } catch { /* corrupt/missing queue */ }
  ...
  try {
    ...
    for (const file of fs.readdirSync(dir)) {
      ...
      try {
        ...
        if (row.verifier) verifierLocks.push(String(row.verifier).toLowerCase());
      } catch { /* malformed lock does not break the probe */ }
    }
  } catch { /* no verify locks */ }

  const pendingVerify = specs.filter(s => ['tests', 'in-progress'].includes(s.stage)
    && s.meta && s.meta.awaiting_verifier === true).length;

  return configured.map(name => {
    const lane = laneConfig(cfg, name);
    const command = lane ? lane.command : '';
    // Argv lanes already have an authoritative executable. Only shell lanes
    // need first-token parsing because their command is intentionally opaque.
    const match = lane && lane.shell
      ? command.match(/^\s*(?:'([^']+)'|"([^"]+)"|(\S+))/)
      : null;
    const binary = lane && !lane.shell
      ? (lane.argv && lane.argv[0]) || ''
      : (match ? (match[1] || match[2] || match[3]) : '');
    const reachable = !!(binary && String(which(binary) || ''));
    const claims = specs.filter(s => ['in-progress', 'tests'].includes(s.stage)
      && String((s.meta && s.meta.claimed_by) || '').toLowerCase() === name).length;
    const runLocked = fs.existsSync(path.join(wsDir, '_metrics', 'locks', name + '.lock'));
    const verifyLocked = verifierLocks.includes(name);
    const queuedExperiments = queueRows.filter(r => r.status === 'queued'
      && String(r.agent_tool || '').toLowerCase() === name).length;
    const busy = claims > 0 || runLocked || verifyLocked;
    const authHint = name === 'claude'
      ? (env.CLAUDE_CODE_OAUTH_TOKEN ? 'oauth token present' : 'check CLAUDE_CODE_OAUTH_TOKEN') : '';
    let next;
    if (!reachable) next = `fix PATH: command not found (${binary || 'missing command'})`;
    else if (busy) next = `${claims} claim${claims === 1 ? '' : 's'}${runLocked ? ' · run lock' : ''}${verifyLocked ? ' · verify lock' : ''}`;
    else if (pendingVerify) next = `${pendingVerify} awaiting verify · tl verify --execute --agent ${name}`;
    else if (queuedExperiments) next = `${queuedExperiments} experiment row${queuedExperiments === 1 ? '' : 's'} · tl experiment drain --agent ${name}`;
    else next = 'idle · no queued verify/experiment work';
    return { name, binary, reachable, busy, state: reachable ? (busy ? 'busy' : 'idle') : 'unreachable', claims, runLocked, verifyLocked, pendingVerify, queuedExperiments, authHint, next };
  });
}

function formatLaneAvailability(rows) {
  if (!rows.length) return ['(no lanes configured)'];
  return rows.map(r => `${r.name.padEnd(10)} ${r.state.padEnd(11)} ${r.next}${r.authHint ? ` · ${r.authHint}` : ''}`);
}
...
module.exports = {
  ...
  installLaunchd,
  automationStatus,
  laneAvailability, formatLaneAvailability,
};
